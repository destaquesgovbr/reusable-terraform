...
resource "google_iap_tunnel_instance_iam_member" "owner_access" {
  count = var.sandbox_config.instance.owner_email != null ? 1 : 0

  project  = var.project_id
  zone     = var.zone
  instance = google_compute_instance.sandbox.name
  role     = "roles/iap.tunnelResourceAccessor"
  member   = "user:${var.sandbox_config.instance.owner_email}"
}

# -----------------------------------------------------------------------------
# Group Access (when group_team_email is set and no owner)
# The entire group can access via IAP (shared sandbox)
# -----------------------------------------------------------------------------

resource "google_iap_tunnel_instance_iam_member" "group_access" {
  count = var.group_team_email != null && var.sandbox_config.instance.owner_email == null ? 1 : 0
